...
- **バランス面の注意(実プレイで確認済み)**: Alpha Animalsは非常に多くの動物(約100種)を同じバイオームの
  ...
  JP_Beastsの当初の値では出現がかなり埋もれてしまうことが実プレイで確認されました。そのため
  `Patches/Patch_BiomeWildSpawn.xml` の全`<commonality>`値を単独導入時の**4倍**に引き上げ済みです。
  それでも出にくいと感じる場合はさらに引き上げてください(値は単なる相対的な重みなので、際限なく
  大きくして構いません)。逆にAlpha Animals等を併用せず単独で使う場合は、出現がやや多く感じられる
  可能性があるので、その場合は値を下げて調整してください。
...
- **自然スポーンがなかなか起きない**
  ...
  段階では1体も出現しないのは珍しくありません。数日プレイしても全く出ない場合は、上記のバランス調整
  (`commonality`引き上げ)を検討してください。
